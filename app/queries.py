...
def adult_filter(show_adult: bool):
    if SAFE_MODE or not show_adult:
        return "is_adult = 0", []
    return "1=1", []

def query(where, params, order, limit, show_adult):
    gate_sql, gate_params = adult_filter(show_adult)
...
def top_all_time(limit=10, show_adult=False, type_=None):
    ...
    return query(where, params, "score DESC, members DESC", limit, show_adult)

def trending_week(limit=10, show_adult=False, type_=None):
    where, params = ("type = ?", [type_]) if type_ else ("1=1", [])
    return query(where, params, "trend_week DESC", limit, show_adult)

def trending_month(limit=10, show_adult=False, type_=None):
    where, params = ("type = ?", [type_]) if type_ else ("1=1", [])
    return query(where, params, "trend_month DESC", limit, show_adult)

def top_airing(limit=10, show_adult=False):
    return query("air_status = 'airing'", [], "score DESC", limit, show_adult)

def top_upcoming(limit=10, show_adult=False):
    return query("air_status = 'upcoming'", [], "members DESC", limit, show_adult)

def newest(limit=10, show_adult=False, type_=None):
    where, params = ("type = ?", [type_]) if type_ else ("1=1", [])
    return query(where, params, "year DESC, id DESC", limit, show_adult)
...
def oldest(limit=10, show_adult=False, type_=None):
    ...
    return query(where, params, "year ASC", limit, show_adult)

def seasonal(season, limit=12, show_adult=False):
    return query("season = ?", [season], "members DESC", limit, show_adult)

def by_type(type_, limit=24, show_adult=False, order="score DESC"):
    return query("type = ?", [type_], order, limit, show_adult)
...
def search(term, limit=30, show_adult=False):
    return query("title LIKE ?", [f"%{term}%"], "members DESC", limit, show_adult)
