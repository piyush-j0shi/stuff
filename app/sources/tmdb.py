...
def genre_names(kind):
    if GENRE_CACHE.get(kind):
        return GENRE_CACHE[kind]
    out = {}
    if enabled():
        with httpx.Client(timeout=15) as c:
            r = get(c, f"{BASE}/genre/{kind}/list", {"api_key": TMDB_API_KEY})
        if r is not None and r.status_code == 200:
            for g in r.json().get("genres", []):
                out[g["id"]] = g["name"]
    if out:
        GENRE_CACHE[kind] = out
    return out


def get(client, url, params, tries=4):
...
def search(term, type_="movie", limit=10):
    ...
    with httpx.Client(timeout=15) as c:
        ...
        r.raise_for_status()
        return [map_result(d, type_) for d in r.json().get("results", [])[:limit]]

def discover(kind, type_, pages, extra=None, on_progress=None):
    if not enabled():
        return []
    out = []
    with httpx.Client(timeout=25) as c:
        for p in range(1, pages + 1):
            ...
            if extra:
                params.update(extra)
            r = get(c, f"{BASE}/discover/{kind}", params)
            if r is None or r.status_code != 200:
                continue
            res = r.json().get("results", [])
            if not res:
                break
            out += [map_result(d, type_) for d in res]
...
def credits(type_, sid, limit=15):
    ...
    kind = "movie" if type_ == "movie" else "tv"
    with httpx.Client(timeout=15) as c:
        r = get(c, f"{BASE}/{kind}/{sid}/credits", {"api_key": TMDB_API_KEY})
    if r is None or r.status_code != 200:
        return {"cast": [], "director": None}
    j = r.json()
...
def popular_movies(pages=5, on_progress=None):
    return discover("movie", "movie", pages, on_progress=on_progress)

def popular_webseries(pages=5, on_progress=None):
    return discover("tv", "webseries", pages, on_progress=on_progress)

def popular_dramas(pages=5, on_progress=None):
    return discover("tv", "drama", pages, {"with_original_language": "ko"}, on_progress)
